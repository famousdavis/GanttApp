...
export const CHANGELOG_ENTRIES: ChangelogEntry[] = [
  ...
  },
  {
    ...
    date: 'September 4, 2026',
    items: [
      <><strong>Fixed</strong> &mdash; duplicating a project with a very long name produced a copy you could not tell apart from the original. If the name was at or near the maximum length the name field allows, the copy came back looking exactly like the original the next time the app loaded.</>,
      <><strong>Note</strong> &mdash; why it happened. Duplicating a project adds a marker to the end of the new name so you can tell the two apart. The name field has a maximum length, and anything longer is trimmed to fit when the app saves and reloads your data. For a name already at that maximum, the marker sat entirely past the limit, so it was trimmed away and the copy came back with a name identical to the original.</>,
      <><strong>Changed</strong> &mdash; the marker is now guaranteed to survive. When a name is long enough that adding the marker would push it past the limit, the name is shortened just enough to make room and the marker is kept. Short names are completely unaffected and duplicate exactly as before.</>,
      <><strong>Fixed</strong> &mdash; a second, quieter half. The check that stops two copies sharing a name was comparing the untrimmed new name against the already-trimmed stored ones, so it could never match, and every duplicate of a long name came out with the same marker number. Shortening now happens before that check rather than after it, so the check compares like with like and the numbering works.</>,
      <><strong>Note</strong> &mdash; the problem grew gradually rather than appearing all at once. Names one character below the limit lost the marker but stayed distinguishable; only at the limit itself did the two names become identical. Names comfortably under the limit were never affected.</>,
    ],
  },
  {
    version: '0.28.19',
  ...
  },
  {
    ...
    date: 'September 3, 2026',
    items: [
      <><strong>Fixed</strong> &mdash; typing into a chart date or label and clicking away threw the edit away. The Gantt chart lets you click a date under a bar, a release name, or a legend label and change it where it sits. Typing a new value and then clicking anywhere else &mdash; another part of the chart, a tab, a snapshot chip &mdash; discarded what you had typed. Only the small tick button, or the Enter key on two of the three, saved anything. Clicking away now saves.</>,
      <><strong>Note</strong> &mdash; the code already knew. The tick button is wired to fire on mouse-down rather than on click, purely so that it beats the handler that was throwing the edit away; without that trick, pressing the tick would have discarded the edit too.</>,
      <><strong>Note</strong> &mdash; dates are where this mattered. A lost release name or legend label is visible immediately, since the old text is still on screen in your own words. A lost date is not: dates render as a bar on a chart that may span many months, so a change of a few days moves it by a few pixels. You could retype it, but only if you noticed, and there is no undo anywhere in this app.</>,
      <><strong>Changed</strong> &mdash; an invalid date is now discarded when you click away, rather than trapping you. Nothing valid is lost, and the editor always closes. If the date cannot be accepted &mdash; a start date after its early finish, say &mdash; clicking away discards it and the original stays. Keeping the editor open would have been worse: it would have followed your cursor around the page, unclosable except by fixing a value you may have been trying to abandon. The tick button and the Enter key are unchanged, and still hold the editor open on an invalid date so you can correct it.</>,
      <><strong>Note</strong> &mdash; the check runs against what is in the box at the moment you leave it. An earlier version of this fix was going to read the &ldquo;there is an error&rdquo; flag instead, which records whether a <em>previous</em> attempt to save had failed and is therefore silent about the value you are actually leaving behind.</>,
      <><strong>Note</strong> &mdash; the cost, stated plainly. An edit you did not mean to make is now permanent. Clicking away commits, this app has no undo, and in cloud mode the change reaches everyone the project is shared with. If you take a snapshot while an edit has just been committed this way, the snapshot records it &mdash; and a snapshot is this app&rsquo;s only historical record. Escape still discards, on all three editors, and now does so on the legend labels through the label&rsquo;s own keyboard handling rather than only through the app-wide one.</>,
      <><strong>Fixed</strong> &mdash; opening a legend label and leaving without typing quietly created a project override. Legend labels have two scopes: a global default set in Settings, and a per-project override. Clicking a label to edit it, then leaving it exactly as you found it, wrote an override anyway. The label looked identical, but it had silently detached from the global default, a reset arrow appeared beside it, and a write went out to cloud storage. This was already reachable by pressing the tick on an unchanged label; making click-away save would have made it reachable by accident. A label committed unchanged &mdash; including one you typed into and then restored &mdash; now writes nothing.</>,
      <><strong>Fixed</strong> &mdash; a legend label editor stayed open on a read-only snapshot. Selecting a saved snapshot puts the chart into a read-only view. Clicking a legend label was already blocked there, but an editor that was <em>already open</em> when you selected the snapshot kept rendering over the frozen chart. All five legend labels now close, matching the release name and the four dates, which have always closed. The architecture notes have claimed since v7.0 that read-only mode disables all inline editing; that claim is true for the first time.</>,
      <><strong>Note</strong> &mdash; what this release did not change. Only clicking away commits; programmatic reloads, a release deleted from another device mid-edit, and Escape all still discard, deliberately. Whether a click on a button blurs a text field is browser-dependent and was not measured here, so nothing above names a specific gesture as guaranteed to save. In every browser the fix is at worst what the app already did.</>,
      <><strong>Note</strong> &mdash; the tests this needed. The two shared editor components were well covered, but the file that renders five of the ten editors &mdash; four dates and a release name for every release on the chart &mdash; had no interaction tests at all. Coverage of its functions went from two of seven to seven of seven, and the legend editor gained its first keyboard and click-away tests. 41 tests were added and two rewritten: both of the rewritten pair had been asserting the defect.</>,
      <><strong>Note</strong> &mdash; a writing convention moved into the file it constrains. Three of the automated checks on the architecture notes work by requiring a retired phrase to be <em>absent</em> from those notes, which makes correcting one of them delicate: a correction written into the notes that quotes the wording it is retiring puts that wording straight back into the file and fails the very check the correction was honouring, with a failure message that looks like a fresh regression. The convention that avoids this &mdash; describe a correction without restating what it replaced &mdash; was set in v0.28.8, but until now was written down only in a project file excluded from version control, so it was absent from every fresh copy of the repository: exactly the state someone is in the first time they meet the problem. It is now stated in full in the check file itself, which is also the one place naming those retired phrases is safe, since the checks read the architecture notes and nothing else. This records the convention; it does not enforce it &mdash; the three checks can only notice a reprint after it has landed, and cannot tell one apart from the original error.</>,
    ],
  },
  {
    version: '0.28.15',
    ...
    items: [
      <><strong>Fixed</strong> &mdash; development tooling only; no application code changed and nothing about how the app behaves is different. Mutation testing makes small deliberate changes to the code and asks whether any test notices. It is expensive, so it runs against three files rather than all of them, and the configuration records why each of the others was left out. Two files were listed as excluded for having too little test coverage, each with a percentage beside it &mdash; and one of those percentages had never described the file it was attached to.</>,
      <><strong>Note</strong> &mdash; it was the second of two figures on a single line of a coverage report, the branch figure and the function figure for the <em>same</em> file sitting side by side, and the second had been read as belonging to the file on the next line. The file it was attributed to has, and has always had, complete coverage of the thing being measured; neither it nor its test has changed in more than a dozen releases. The exclusion is still right, for a reason the configuration already gives elsewhere &mdash; the file is small and fully covered, so mutating it would return almost nothing. Only the stated reason was wrong.</>,
      <><strong>Fixed</strong> &mdash; a second figure in the same sentence, out of date since the previous release. The other excluded file&rsquo;s coverage rose from 58% to 79% in v0.28.14, when five unused functions were deleted from it, which clears the threshold its exclusion cited. Both exclusions still stand and the scope is unchanged; whether that file should now come into scope is a separate decision this release does not make, and the note says so explicitly so that a later reader cannot mistake a corrected number for a decision.</>,
      <><strong>Fixed</strong> &mdash; a cross-reference that pointed at a document which does not contain what it promised. The configuration told anyone reading it to see the mutation-testing report for what was excluded and why; that report has never contained the name of either excluded file and documents no exclusion at all, recording instead the run itself &mdash; the score, every surviving change and how each was classified, and what the run cost. The reference was false on the day it was written and survived seven releases, because a pointer to a document is exactly the kind of claim nothing ever executes. It now says where the information actually is.</>,
      <><strong>Added</strong> &mdash; a check that the mutation scope and its written record cannot drift apart. Every file the configuration puts in scope must exist, since a renamed or deleted entry would otherwise shrink the scope silently &mdash; the tool treats those entries as patterns, and a pattern matching nothing raises no error while quietly lowering how much code is under test. And every file in scope must appear in the written record of the run, so the record cannot describe a run nobody could reproduce from it.</>,
      <><strong>Note</strong> &mdash; stated plainly, because it is the same weakness as the checks added in the last two releases: this reads names, not prose. A percentage going stale, which is what this release fixed twice, is invisible to it.</>,
      <><strong>Changed</strong> &mdash; the project&rsquo;s own working notes described a snapshot utility file as handling snapshot creation, validation and browser storage. Only the validation half survived the previous release, and the entry now names the two things the file actually still does. That file is excluded from version control, so the change appears in no comparison of what changed and is covered by no automated check beyond the two version numbers the release gate verifies; it was made by hand and recorded in the release notes.</>,
    ],
  },
  {
    version: '0.28.14',
    date: 'September 1, 2026',
    items: [
      <><strong>Removed</strong> &mdash; internal cleanup; no application code path changed and nothing about how the app behaves is different. A utility file for chart snapshots exported seven functions and five of them had no callers anywhere in the project. They wrote snapshots straight into browser storage, which is how the app worked before it gained a storage layer that chooses between your browser and the cloud depending on which you have picked. Once that layer arrived the five became unreachable, and they had sat there since.</>,
      <><strong>Note</strong> &mdash; they did not look unused, which is why they lasted. The live storage layer declares operations with exactly the same names, so searching for any one of them returns dozens of results &mdash; the interface, both implementations, and every place the app saves or deletes a snapshot. Every one of those is the storage layer; none of them is this file.</>,
      <><strong>Removed</strong> &mdash; three settings the five functions needed and nothing else did: the name under which snapshots were filed in browser storage, and two limits on how many snapshots may be kept. The limits already exist in one other place that calls itself the single source of truth, so the copies were a second answer to a question that should only have one.</>,
      <><strong>Added</strong> &mdash; a check that the browser-storage key cannot come back. Removing a function something still uses is caught at once, because the project stops building; removing a setting is caught by nothing. This project has no rule reporting an unused setting and no build option that would fail on one, so all three deleted settings were invisible to every automated check the release runs, in both directions &mdash; leaving them in and taking them out looked identical. The key is now pinned by name, because anything writing snapshots under it is going around the storage layer and would quietly disagree with cloud storage.</>,
      <><strong>Changed</strong> &mdash; one line of the architecture notes. They described that file as handling snapshot creation, validation and browser storage; only the validation half is true now, and the line names the two things the file actually still does.</>,
      <><strong>Added</strong> &mdash; a check that points the opposite way from every other check on those notes: it requires text to be <em>present</em>. The notes correctly list the storage layer&rsquo;s operations, which share their names with the five deleted functions, so anyone searching the notes for the deleted names finds nine lines of accurate documentation and is invited to delete them while believing they are correcting an error. One of those nine lines is now pinned, so deleting it fails the release.</>,
      <><strong>Note</strong> &mdash; both new checks look for exact wording, like the two added in the previous release, and share the same weakness: reword a stale claim and it passes. They guard the specific sentences this release touched and nothing beyond them. The key check likewise looks for one exact word in one file; the same code written with a different key, or placed elsewhere, passes.</>,
      <><strong>Added</strong> &mdash; the first tests for the keyboard-shortcuts handler, which is wired into three screens, listens for every key press across the whole window, cancels the browser&rsquo;s own response when it recognises a combination, and takes over Ctrl+S and Cmd+S so the browser&rsquo;s save dialog does not open. It had never once run in a test: one of its twenty-one possible outcomes was covered, and that one was the default value of an argument. All twenty-one are covered now, and the handler itself was not changed.</>,
      <><strong>Note</strong> &mdash; four of those behaviours are worth naming, being easy to get wrong and invisible from outside. Escape reaches you even while you are typing in a text field, because it is checked before the rule that ignores shortcuts in text fields. Escape does not cancel the browser&rsquo;s own response, while every other recognised shortcut does. Both of those hold only when an Escape shortcut has actually been registered &mdash; without one, Escape is treated as an ordinary key, so a combination like Ctrl+Escape <em>does</em> cancel it, contradicting them in the case nobody thinks to check. And combinations match in one fixed order, Ctrl then Shift then the key; written the other way round a shortcut can never match.</>,
      <><strong>Note</strong> &mdash; measured coverage of the snapshot utility file rose because its untested half is gone, not because anything new was tested there: statements from 41% to 84%, branches from 58% to 79%. That clears the coverage threshold recorded as the reason for leaving the file out of the project&rsquo;s mutation-testing scope. It does not meet the higher standard the files actually in that scope were held to, so this release makes no claim that the file is now a candidate; that decision is separate and unchanged.</>,
    ],
  },
  {
    version: '0.28.13',
  ...
  },
  {
    ...
    date: 'August 19, 2026',
    items: [
      <><strong>Changed</strong> &mdash; signing in with Microsoft now requires a work or school account. Personal Microsoft accounts &mdash; outlook.com, hotmail.com, live.com &mdash; are no longer accepted. Microsoft itself enforces this, so such an account is refused at the sign-in screen before any password is entered. The change was made for institutions evaluating the Suite, who reasonably expect &ldquo;sign in with Microsoft&rdquo; to mean an organisational account rather than any account at all.</>,
      <><strong>Changed</strong> &mdash; nothing changes for personal use: Google still accepts personal accounts. The cloud storage window and the Settings page now say so, rather than letting you choose Microsoft and discover the restriction from an error message.</>,
    ],
  },
  {
    version: '0.28.8',
